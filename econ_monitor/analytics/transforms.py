...
All functions operate on pandas Series with a DatetimeIndex.
"""
...
def yoy_pct(s: pd.Series, periods: int = 12) -> pd.Series:
    """Year-over-year percent change. Default assumes monthly data (12 periods)."""
    return s.pct_change(periods=periods) * 100


def qoq_pct(s: pd.Series) -> pd.Series:
    """Quarter-over-quarter percent change (for quarterly data)."""
    return s.pct_change() * 100
...
def trend_direction(s: pd.Series, window: int = 6) -> str:
    """Classify the recent trend as 'improving', 'stable', or 'deteriorating'.

    Based on the slope of a linear regression over the last `window` observations.
    """
    ...
    # Normalize to make slope threshold meaningful
    y_range = y.max() - y.min()
    if y_range == 0:
        return "stable"
...
def apply_transform(s: pd.Series, transform: str, frequency: str = "monthly") -> pd.Series:
    """Apply the indicator's configured transform to raw data."""
